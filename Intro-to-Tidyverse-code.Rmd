...
`rename()`: you can also rename columns in dplyr. The new name is first and the original name is second. 
...
1) First, we are going to repeat the command we created above to select only the dolphin's name and all four possible date values. 

2) Next, we will filter out all individuals who don't have a status date. !is.na(statusDate)

3) Next we will filter out all inividuals whose transfer date is earlier than 1990 (keep only transfers after Jan 1, 1990). It's great that the "filter" command actual works with date values! 
...
4) We can use the variety of functions in summarize to create a summary dataframe from our original dataset. Note, this dataframe will "overwrite" your original dataset if you save it as the same object name. For example, you'd want to name this acq_sex_table or something. 
...
If you define the aesthetics in the ggplot() command, the geoms don't require any arguments, but you can always add layer-specific aesthetics. 

`Base equivalent:`

```{r add geoms}
p1<-ggplot(data = gapminder, aes(x = year, y = pop, color = continent)) + geom_point() + 
...
ggplot() + geom_point(data = gapminder, aes(x = gdpPercap, y = lifeExp, color = continent, shape = continent)) + 
...
```{r top emitters pretty, message=FALSE}
...
#### Step only if you _absolutely_ must.... 
